...
def predict_func(model, loader, main_labels):
    ...
    for i, sample in enumerate(loader):
        ...
        for in_img, prediction in zip(sample[0], predicted):
            ...
            conv = tmp(in_img)
            path_out = '../data/fl27/segmented_resized/predicted'
            f_name = main_labels[prediction] + '_' + str(uuid.uuid4().hex) + '.jpg'
            path = os.path.join(path_out, f_name)
            io.imsave(path, conv)
def main():
    #Path for model
    model_path = '../train_cnn/cnn-2.pt'
	# Get labels for showing results
    main_annotation = '../data/fl27/crop_annotation.txt'
    ...
    segmented_img_path = '../data/fl27/segmented'
    # Path for segmented images dataset annotation
    segment_set_path = '../annotations/segmentset.txt'
    # Path for segmented, resized and combined images
    segmented_resized_img_path = '../data/fl27/segmented_resized/not_predicted'
